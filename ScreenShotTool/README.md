...
```powershell
dotnet build .\ScreenShotTool.csproj
dotnet publish .\ScreenShotTool.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=false
```
...
```text
artifacts\installer\ScreenShotToolSetup.exe
```
...
```powershell
.\artifacts\installer\ScreenShotToolSetup.exe /silent /no-launch
```

For unattended install with startup and a Desktop shortcut:

```powershell
.\artifacts\installer\ScreenShotToolSetup.exe /silent /startup /desktop
```
...
See the repository root `CONTRIBUTING.md`.

## Security

See the repository root `SECURITY.md` for vulnerability reporting.

## License

ScreenShotTool is released under the MIT License in the repository root `LICENSE` file.
